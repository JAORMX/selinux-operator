# Operator variables
# ==================
export APP_NAME=selinux-operator
...
UDICA_IMAGE_PATH?=$(IMAGE_REPO)/udica
...
.PHONY: all
all: build ## Test and Build the selinux-operator
...
.PHONY: udica-image 
udica-image:
	$(RUNTIME) build -t $(UDICA_IMAGE_PATH):$(TAG) -f ./images/udica/Dockerfile .

.PHONY: build
build: ## Build the selinux-operator binary
	$(GO) build -o $(TARGET) github.com/JAORMX/selinux-operator/cmd/manager
...
.PHONY: run
run: operator-sdk ## Run the selinux-operator locally
...
# probably want to push the selinux-operator image to the cluster we're
...
images-to-cluster: namespace openshift-user images
	...
	IMAGE_REGISTRY_HOST=$$(oc get route default-route -n openshift-image-registry --template='{{ .spec.host }}'); \
		...
		$(RUNTIME) push --tls-verify=false $(UDICA_IMAGE_PATH):$(TAG) $${IMAGE_REGISTRY_HOST}/$(NAMESPACE)/udica:$(TAG)
